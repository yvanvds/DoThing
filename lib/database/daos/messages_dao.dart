...
class MessagesDao extends DatabaseAccessor<AppDatabase>
  ...
  Future<List<ContactInboxSummary>> getInboxContactSummaries({
    ...
    final sourceFilterClause = source == null ? '' : 'm.source = ? AND';
    final variables = <Variable<Object>>[];
    if (source != null) {
      variables.add(Variable<String>(source));
    }
  ...
  Future<List<SmartschoolMessageHeader>> getInboxHeadersForContact(
    ...
    final sourceFilterClause = source == null ? '' : 'AND m.source = ?';
    ...
    final rows = await customSelect(
      ...
      WHERE
        ...
        1 = 1
        $sourceFilterClause
        AND m.mailbox IN ('inbox', 'sent') AND
        m.is_deleted = 0 AND
        m.is_archived = 0
        $selfFilterClause
